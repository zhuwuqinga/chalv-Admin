...
export default {
  ...
  data() {
    return {
      ...
      form1: { returnNo:undefined,returnStatus: undefined, returnAuthReason: undefined },
      options:[{
        value: '0',
          label: '待审核'
        }, {
  ...
  methods: {
    downloadExcel() {
      this.$confirm(`确定导出查询的数据吗?`, "提示", {
        ...
        request({
          url: url.v1.hotel.refudExportExcel,
          method: "post",
          data: { ...searchData },
          ...
          aLink.href = url;
          aLink.setAttribute("download", "酒店退款订单列表excel.xls");
    ...
    marketList() {
      ...
      request({
        //退款订单列表
        url: url.v1.hotel.selectHotleRefundList,
        method: "post",
        ...
        res.data.list.forEach((element) => {
          if (element.productFaceImage != "" && element.productFaceImage!=null) {
            element.productFaceImage = element.productFaceImage.split("!#!");
          }
        });
      });
    },
    ...
    showReturnDialog(row) {
      this.form1.returnNo = row.refundNo;
      this.form1.returnStatus = "2";
      this.dialogReturnResouz = true;
    },
    confirmToBlack(row) {
      request({
        url: url.v1.hotel.hotelReturnOrder,
        method: "POST",
        data: this.form1,
      }).then((res) => {
        if (res.status === 200) {
