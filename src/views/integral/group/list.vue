      ...
      option: {
        ...
        column: [
          ...
          },
          {
            ...
            type: "select",
            cascaderIndex: 0,
            dicUrl: `${url.v1.integralGroup.listTree}/{{key}}`,
            display: false,
          },
  ...
  methods: {
    ...
    rowDel(form) {
      this.$confirm("此操作将永久删除该规格组, 是否继续?", "提示", {
        ...
        .then(() => {
          request({
            url: url.v1.integralGroup.delete,
            method: "POST",
            data: {
              ids: form.id,
            },
    ...
    rowUpdate(form, index, done, loading) {
      request({
        url: url.v1.integralGroup.update,
        method: "POST",
        data: form,
      }).then((res) => {
        if (res.status == 404) {
