...
export const formNullToEmpty = (object) => {
    var newObject = {};
    if (validatenull(object)) return newObject;
    for (const key in object) {
        if (object.hasOwnProperty(key)) {
            if (!validatenull(object[key])) {
                if (object[key] === true) {
                    newObject[key] = 1;
...
export const calcDescartes = function(array) {
...
export const download = (url) => {
    ajax(url, function (xhr) {
        downloadFile(xhr.response, UrlFileName(url))
    }, {
        responseType: 'blob'
    })
